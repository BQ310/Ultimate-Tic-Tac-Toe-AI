...
while not board.is_ended(state):
    # this updates the board based on the previous action
    print(board.display(state, last_action))
    # this is just getting the player number
    print("Player "+str(board.current_player(state)))
    # this is calling the think function and gets an action
    # think function runs on unique states
    last_action = current_player(board, state)
    # get the new state based on the action taken by think / player
